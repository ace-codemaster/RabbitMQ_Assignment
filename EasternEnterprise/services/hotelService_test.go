...
func TestSaveHotel_WithProper(t *testing.T) {
	...
	parameter := &model.Transmission{}
	err := json.Unmarshal([]byte(jsonStr), parameter)
	if err != nil {
		t.Errorf("Error..!!")
		return
	}

	status := SaveHotelData(parameter)
	if !status {
		t.Errorf("Required Result Was True")
	}
}

func TestSaveHotel_WithNil(t *testing.T) {

	status := SaveHotelData(nil)
	if status {
		t.Errorf("Required Result Was False")
	}
}
